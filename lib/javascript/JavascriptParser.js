...
class JavascriptParser extends Parser {
	 ...
	 * @param {(Statement | ModuleDeclaration)[]} statements statements
	 * @returns {boolean} whether the statements contains dead control flow
	 */
	walkStatements(statements) {
		let deadControlFlow = false;
		for (let index = 0, len = statements.length; index < len; index++) {
			const statement = statements[index];
			if (this.walkStatement(statement) && this.scope.topLevelScope !== true) {
				// we've met dead control flow
				// eg.
				// 	{
				//		if (true) {
				//			return;
				...
				//		... <- skip the rest
				//	}
				deadControlFlow = true;
				break;
			}
		}
		return deadControlFlow;
	}
	...
	/**
	 * @param {Statement | ModuleDeclaration} statement statement
	 * @returns {boolean} whether the statement contains dead control flow
	 */
	walkStatement(statement) {
		...
		switch (statement.type) {
			case "BlockStatement":
				deadControlFlow = this.walkBlockStatement(statement);
				break;
			case "ClassDeclaration":
			...
			case "FunctionDeclaration":
				...
				break;
			case "IfStatement":
				deadControlFlow = this.walkIfStatement(statement);
				break;
			case "LabeledStatement":
			...
			case "ThrowStatement":
				...
				break;
			case "TryStatement":
				deadControlFlow = this.walkTryStatement(statement);
				break;
			case "VariableDeclaration":
	...
	walkNestedStatement(statement) {
		this.prevStatement = undefined;
		return this.walkStatement(statement);
	}
	...
	walkBlockStatement(statement) {
		...
		this.inBlockScope(() => {
			...
			deadControlFlow = deadControlFlow || this.walkStatements(body);
		});
		return deadControlFlow;
	}
	...
	walkIfStatement(statement) {
		...
		if (result === undefined) {
			...
			const left = this.walkNestedStatement(statement.consequent);
			let right = false;
			if (statement.alternate) {
				right =
					deadControlFlow || this.walkNestedStatement(statement.alternate);
			}
			deadControlFlow = left && right;
		} else if (result) {
			deadControlFlow = this.walkNestedStatement(statement.consequent);
	...
	walkTryStatement(statement) {
		let deadControlFlow = false;
		if (this.scope.inTry) {
			deadControlFlow = this.walkStatement(statement.block);
		} else {
		...
		if (statement.handler) {
			// if has catch, we should ignore deadControlFlow in try block
			// eg. try {throw 1} catch(){}; require() <- should walk this
			deadControlFlow = false;
